...
|---|---|---|---|
| CLI | `src/aegis/cli.py` | Typer `aegis` with `eval`, `validate`, `rosbench` | Backend select `mujoco\|isaaclab` (`:74`), overrides CLI>YAML>defaults, new VRAM-safe `--headless --quantize` flags (`:62-63`), exits 0/2/3 |
...
| Metrics/report | `src/aegis/eval/metrics.py`, `report.py` | p50/p95 latency, `report.json` contract (run_id, task_counts, violations, recoveries, gpu_hours, recommendation 4 rules, warnings) | `gpu_hours` = per-kernel `inference_s` sum when `cuda` else `0` |
| MuJoCo env | `src/aegis/envs/mujoco_pick_place.py` | Menagerie Franka `panda_vel.xml`, 7 vel + tendon gripper `0..255`, torque-PD, 3 cams `256×256` at chunk boundaries, `Env` protocol `reset/step/observe/render_images/state_snapshot` | ~12.5 KB, largest env file; contact-force grasp logic |
...
| Tests | `tests/test_eval.py`, `test_policy_model.py`, `test_seed_sweep.py` (20 tests per PRD/README) | Config exit-2, determinism, random-fails/scripted-succeeds, budget/adapter/SmolVLA config, 5 seeds×5 eps ≥90% | See §6 note on local runner |
...
Test note (2026-09-14 check): `python -m pytest -q` with system Python 3.13 **fails collection** (`ModuleNotFoundError: No module named 'aegis'`) — expected, must run via `uv run pytest` / `uv sync` venv (Python 3.12 per `pyproject.toml`, `torch cu128` Windows wheel). Canonical suite is 20 tests; CI badge claims 20 passed. `git status` clean; HEAD `b2f2941` (VRAM-safe USD checklist).
...
5. **Housekeeping:** pin `uv run pytest` green in this checkout, refresh `pytest_output.txt`, resolve `torch cu128` Windows-vs-WSL2 env split, keep `PRD.md` as single source (update on any deviation).
...
- Recommendation is 4 hand-written rules, not learned.
